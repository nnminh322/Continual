#!/bin/bash
###############################################################################
# OT-SIGN + GainLoRA (T5-Large) — Order 4 (Long Benchmark)
# ==========================================================
# Task order: mnli → cb → wic → ... → yahoo (15 tasks)
# Benchmark:  Long (Exact Match metric)
# Hardware:   2× T4 16GB  |  Estimated time: ~3-4h
# Run:        bash run_ot_sign_order4_t5large.sh [GPU_IDS] [MODEL_PATH]
###############################################################################
...
# ── Config ──────────────────────────────────────────────────────────────────
RUN_NAME="ot_sign_order4_t5large"
OUTPUT_BASE="logs_and_outputs/${RUN_NAME}/outputs"
CONFIG_PREFIX="configs/gen_script_long_order4_t5_configs"
...
mkdir -p "${OUTPUT_BASE}"
echo "════════════════════════════════════════════════"
echo "  OT-SIGN+GainLoRA  |  Order 4  |  T5-Large"
