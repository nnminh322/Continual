...
def main():
    ...
    if training_args.data_replay_freq != -1:
        ...
        replay_dataset_dict, replay_label_dict = None, None
        if model_args.load_checkpoint_from and training_args.data_replay_freq != -1:
            replay_dataset_dict = {}
            for idx in range(cur_task_id):
                replay_builder = CLInstructions(
                    data_dir=os.path.abspath(data_args.gen_data_dir) if data_args.gen_data_dir else None,
                    task_config_dir=os.path.abspath(task_config[task_order[idx]]),
                    max_num_instances_per_task=data_args.max_num_instances_per_task,
                    max_num_instances_per_eval_task=data_args.max_num_instances_per_eval_task,
                    num_examples=data_args.num_examples,
                )
                replay_builder.download_and_prepare()
                raw_datasets_gen = replay_builder.as_dataset()
    ...
    if training_args.do_predict:
        ...
        output_root = os.path.dirname(os.path.normpath(training_args.output_dir))
        os.makedirs(output_root, exist_ok=True)
        with open(os.path.join(output_root, "task_order.txt"), 'w') as f:
