...
def resolve_gpt2tokenizer_path():
    candidates = [
        GPT2TOKENIZER,
        os.path.abspath(os.path.join(CURRENT_DIR, "../../..", "root_gainlora", "data", "gpt2tokenizer")),
        os.path.abspath(os.path.join(CURRENT_DIR, "../../..", "new_gainlora", "data", "gpt2tokenizer")),
    ]
    for candidate in candidates:
        if os.path.isdir(candidate):
            return candidate
    raise FileNotFoundError(f"Could not find gpt2tokenizer in any known location: {candidates}")


class GPTTokenizer:
    gpt_tokenizer = AutoTokenizer.from_pretrained(resolve_gpt2tokenizer_path(), max_length=1e5)
