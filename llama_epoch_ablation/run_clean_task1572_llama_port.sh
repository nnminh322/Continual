...
PORT_BASE="$SCRIPT_DIR/llama_clean_port"
...
GPU_IDS="0"
MASTER_PORT="49500"
RUN_NAME="clean_task1572_llama_gainlora_srt_cleanport"
...
{
    echo "[CLEAN-T1572] log_file=$LOG_FILE"
    echo "[CLEAN-T1572] model_path=$MODEL_PATH"
    echo "[CLEAN-T1572] gpu_ids=$GPU_IDS"
    echo "[CLEAN-T1572] master_port=$MASTER_PORT"
    echo "[CLEAN-T1572] deepspeed_launcher=$DEEPSPEED_LAUNCHER"
    echo "[CLEAN-T1572] output_dir=$OUT_DIR"
    printf '[CLEAN-T1572] command='
    printf '%q ' "${CMD[@]}"
    printf '\n'
} | tee "$LOG_FILE"
...
echo "[CLEAN-T1572] done. See $LOG_FILE"
