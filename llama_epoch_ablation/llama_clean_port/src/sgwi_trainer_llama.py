...
class SGWI_DualFisher_LLaMA_Trainer(GainLoRA_OLoRA_Trainer):
    ...
    def _compute_sgwi_weights(self) -> Dict[int, float]:
        ...
        for task_id, sig in self.srt_router.signatures.items():
            ...
            pooled_cov = getattr(self.srt_router, 'pooled_cov', None)
            if pooled_cov is None:
                pooled_cov = getattr(self.srt_router, '_Sigma_pool', None)
            if pooled_cov is not None:
                try:
                    cov_inv = np.linalg.inv(pooled_cov + 1e-6 * np.eye(len(diff)))
