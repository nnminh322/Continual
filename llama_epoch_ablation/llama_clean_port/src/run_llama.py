...
def main():
    ...
    # NOTE: overwrite_output_dir was removed in transformers 5.0.
    # For continual learning, always look for a checkpoint to resume — this is the intended behavior.
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train:
        ...
        if last_checkpoint is not None and training_args.resume_from_checkpoint is None:
            logger.info(
                ...
                "the `--output_dir` or set `--resume_from_checkpoint false`."
            )
    ...
    if model_args.load_checkpoint_from:
        ...
        model.model.trans_input.load_state_dict(current_trans_state)
        ...
        if training_args.model_name == 'gainlora' and hasattr(model.model, 'previous_trans_input'):
            ...
            model.model.previous_trans_input.input_linear = nn.Parameter(
                previous_input_linear.detach().clone().to(dtype=model.model.trans_input[0].weight.dtype),
                requires_grad=False,
            )
            model.model.previous_trans_input.output_linear = nn.Parameter(
                previous_output_linear.detach().clone().to(dtype=model.model.trans_input[2].weight.dtype),
                requires_grad=False,
            )
    ...
    if training_args.model_name == 'olora':
        ...
        # Mirror the T5 path: gainlora always goes through the SGWI-capable trainer,
        # with SRT/SGWI behavior toggled internally by arguments.
        ...
        print(f"[C2-LLaMA] sgwi={training_args.sgwi} → mode={_sgwi_mode}, "
              ...
              f"use_srt_router={training_args.use_srt_router}")
