...
class T5PreTrainedModel(PreTrainedModel):
    ...
    # NOTE: _set_gradient_checkpointing removed intentionally to fix OOM leak in Autograd.
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, prompt_config=None):
        ...
        if not self.is_decoder and not prompt_config["run_single"]:
            ...
            # ── SRT Router integration ─────────────────────────────────
            self.srt_router = None          # set externally by SRT_Trainer
            self.use_srt_routing = False   # enable SRT routing at inference
            self.srt_n_tasks = 0            # number of registered tasks
            self.srt_emb_buffer = []       # buffer for embedding extraction
            self.srt_task_id_to_idx = {}   # maps srt_router task_id → index in key_attention_weights
            ...
            if prompt_config["previous_prompt_key_path"] is not None and prompt_config["task_id"]:
                ...
                expected_shape = (prompt_config["task_id"], config.d_model)
