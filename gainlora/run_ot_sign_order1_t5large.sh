#!/bin/bash
###############################################################################
# OT-SIGN + GainLoRA (T5-Large) — Order 1 (SuperNI)
# ===================================================
# Task order: samsum → sst2 → xsum → ... → emotion (15 tasks)
# Benchmark:  SuperNI (RougeL metric)
# Hardware:   2× T4 16GB  |  Estimated time: ~9-10h
# Run:        bash run_ot_sign_order1_t5large.sh [GPU_IDS] [MODEL_PATH]
###############################################################################
...
# ── Config ──────────────────────────────────────────────────────────────────
RUN_NAME="ot_sign_order1_t5large"
OUTPUT_BASE="logs_and_outputs/${RUN_NAME}/outputs"
CONFIG_PREFIX="configs/gen_script_superni_order1_t5_configs"
GEN_DATA_DIR="generated_data/lora_gen_superni_t5"
METRIC_TYPE="rougeL"   # RougeL for SuperNI
KL_RATIO=0.5
EPOCHS=50
...
mkdir -p "${OUTPUT_BASE}"
echo "════════════════════════════════════════════════"
echo "  OT-SIGN+GainLoRA  |  Order 1  |  T5-Large"
...
for i in "${!TASKS[@]}"; do
  ...
  # Metric key (task 1 has no "for_task" suffix)
  if [[ $i -eq 0 ]]; then
  ...
  # Build previous-task flags (empty for task 1)
  PREV_FLAGS=""
  if [[ $i -gt 0 ]]; then
  ...
  # Accumulate previous lora paths
...
echo "  Compare with: results/comparison_results.md"
