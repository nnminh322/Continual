...
try:
    ...
    pass  # ipdb not required for production runs
...
# OT-SIGN imports
from ot_sign_utils import sinkhorn_log, vmf_cost_matrix
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, prompt_config, has_relative_attention_bias=False):
        ...
        if prompt_config["previous_lora_path"] is not None and prompt_config["task_id"]:
            with torch.no_grad():
    ...
    def get_chunk(self, chunk):
        ...
        self.index, self.step = chunk, self.d_model // chunk
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, prompt_config=None):
        ...
        if not self.is_decoder and not prompt_config["run_single"]:
            ...
            if prompt_config["previous_prompt_key_path"] is not None and prompt_config["task_id"]:
                ...
                # self.previous_trans_input.load_state_dict()
            ...
            # ===== OT-SIGN: vMF signatures + OT routing =====
            self.use_ot_routing = prompt_config.get('use_ot_routing', False)
            self.ot_epsilon = prompt_config.get('ot_epsilon', 0.05)
            self.ot_n_iter = prompt_config.get('ot_n_iter', 10)
            self.default_kappa = prompt_config.get('default_kappa', 10.0)
            self.vmf_signatures = {}  # {task_id: (mu, kappa)}
            self.last_x_normalized = None  # stored for anti-invasion in trainer

            # Load vMF signatures from previous tasks if available
            vmf_sig_path = prompt_config.get('previous_vmf_signatures_path', None)
            if vmf_sig_path is not None and os.path.exists(vmf_sig_path):
                print("----------Loading Previous vMF Signatures----------")
                loaded_sigs = torch.load(vmf_sig_path, map_location='cpu')
                self.vmf_signatures = loaded_sigs
                print(f"  Loaded {len(loaded_sigs)} vMF signatures")
            # ===== End OT-SIGN init =====
    ...
    def cal_attention_ot(self, x_normalized):
        """
        OT-SIGN routing using vMF signatures + Sinkhorn OT.
        
        Args:
            x_normalized: Normalized input features (B, 1, d)
            
        Returns:
            weights: Transport plan (B, N_total, 1) — same shape as original cal_attention
        """
        x = x_normalized.squeeze(1)  # (B, d)
        
        # Build cost matrix from vMF signatures + current task's prompt_key
        C = vmf_cost_matrix(
            x, self.vmf_signatures,
            prompt_key=self.prompt_key,
            default_kappa=self.default_kappa
        )  # (B, N_total)
        
        # Sinkhorn OT (log-domain for FP16 stability)
        weights = sinkhorn_log(
            C.float(),  # compute in FP32 for stability
            epsilon=self.ot_epsilon,
            n_iter=self.ot_n_iter
        ).to(dtype=x.dtype)  # (B, N_total)
        
        return weights.unsqueeze(2)  # (B, N_total, 1)
    ...
    def forward(
        ...
        if not self.is_decoder and not self.prompt_config["run_single"]:
            ...
            self.last_x_normalized = None  # reset per forward
            prompt_key = self.prompt_key

            # ===== OT-SIGN routing branch =====
            if self.use_ot_routing and len(self.vmf_signatures) > 0:
                # Use SINGLE trans_input for all tasks (no previous_trans_input needed)
                ...
                # Normalize for vMF routing
                x_normalized = F.normalize(x, dim=-1)  # (B, 1, d)
                self.last_x_normalized = x_normalized.detach()  # store for anti-invasion
                
                # OT routing with vMF signatures
                key_attention_weights = self.cal_attention_ot(x_normalized)  # (B, N_total, 1)
            ...
            # ===== Original routing branch (fallback) =====
            elif self.previous_prompts_keys is not None:
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def memory_replay(self, input_ids, replay_labels, attention_mask):
        kl_loss = None
