...
from compat_transformers import ShardedDDPOption, is_sagemaker_mp_enabled, smp_forward_backward, deepspeed_init, deepspeed_load_checkpoint, HPSearchBackend, patch_trainer_compat, is_torch_tpu_available, ALL_LAYERNORM_LAYERS  # explicit for transformers 5.x
...
# CuPy import with fallback for environments without CUDA CuPy
try:
    import cupy as cp
...
# OT-SIGN imports
from ot_sign_utils import (
    compute_vmf_params,
    compute_anti_drift_loss_cached,
    compute_anti_invasion_loss,
    CachedRefTransInput,
)
...
class GainLoRA_InfLoRA_Trainer(Seq2SeqTrainer):
    ...
    def __init__(self, model, args, train_dataset, cur_task_id, task_order, data_collator_replay=None, replay_dataset_dict=None, replay_label_dict=None, eval_dataset=None, tokenizer=None, data_collator=None, compute_metrics=None, callbacks=None):
        ...
        # ===== OT-SIGN: Initialize anti-drift reference =====
        self.use_ot_sign = getattr(args, 'use_ot_routing', False)
        self.ref_trans_input_cached = None
        if self.use_ot_sign and cur_task_id > 0:
            # Create frozen snapshot of trans_input from loaded checkpoint
            try:
                self.ref_trans_input_cached = CachedRefTransInput(model.encoder.trans_input)
                self.ref_trans_input_cached = self.ref_trans_input_cached.to(next(model.parameters()).device)
                print("[OT-SIGN] Created reference trans_input snapshot for anti-drift")
            except Exception as e:
                print(f"[OT-SIGN] Warning: Could not create ref_trans_input: {e}")
                self.ref_trans_input_cached = None
        # ===== End OT-SIGN init =====
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # ===== OT-SIGN: Anti-invasion loss (add to CE loss before backward) =====
        if self.use_ot_sign and hasattr(self.model, 'encoder') and len(self.model.encoder.vmf_signatures) > 0:
            x_norm = self.model.encoder.last_x_normalized  # set during forward
            if x_norm is not None:
                # Need fresh x_norm with gradient for anti-invasion
                avg_emb = (inputs.get('attention_mask', None).unsqueeze(-1) * self.model.encoder.embed_tokens(inputs['input_ids'])).mean(dim=1, keepdim=True)
                medium = self.model.encoder.trans_input[1](self.model.encoder.trans_input[0](avg_emb))
                x_fresh = self.model.encoder.trans_input[3](self.model.encoder.trans_input[2](medium))
                x_fresh_norm = F_loss.normalize(x_fresh.squeeze(1), dim=-1)
                
                inv_loss = compute_anti_invasion_loss(
                    x_fresh_norm,
                    self.model.encoder.vmf_signatures,
                    self.cur_task_id,
                    threshold=getattr(self.args, 'invasion_threshold', 2.3)
                )
                lambda_inv = getattr(self.args, 'lambda_inv', 0.001)
                loss = loss + lambda_inv * inv_loss
        # ===== End anti-invasion =====
        ...
        if self.state.global_step > self.args.replay_after_n_epoch*self.args.step_per_epoch and self.args.data_replay_freq != -1 and self.state.global_step % self.args.data_replay_freq == 0:
            for item in self.replay_iterator_dict.keys():
                ...
                # ===== OT-SIGN: Anti-drift loss (on replay data) =====
                if self.use_ot_sign and self.ref_trans_input_cached is not None:
                    drift_loss = compute_anti_drift_loss_cached(
                        self.model.encoder,
                        self.ref_trans_input_cached,
                        replay_inputs["input_ids"],
                        replay_inputs.get("attention_mask", torch.ones_like(replay_inputs["input_ids"]))
                    )
                    lambda_drift = getattr(self.args, 'lambda_drift', 0.01)
                    scaled_drift = lambda_drift * drift_loss
                    if self.do_grad_scaling:
                        self.scaler.scale(scaled_drift).backward()
                    else:
                        scaled_drift.backward()
                # ===== End anti-drift =====

        return loss.detach()

    # ===== OT-SIGN: vMF signature computation =====
    def compute_vmf_signature(self, task_id):
        """
        Compute vMF signature for the current task after training completes.
        Runs a single pass over training data to collect trans_input features,
        then fits vMF parameters (mu, kappa).
        ...
        Returns:
            (mu, kappa) tuple
        """
        print(f"[OT-SIGN] Computing vMF signature for task {task_id}...")
        self.model.eval()
        all_x = []
        
        train_dataloader = self.get_train_dataloader()
        max_batches = 200  # limit to avoid excessive computation
        ...
        with torch.no_grad():
            for i, batch in enumerate(train_dataloader):
                ...
                medium = self.model.encoder.trans_input[1](self.model.encoder.trans_input[0](avg_emb))
                x = self.model.encoder.trans_input[3](self.model.encoder.trans_input[2](medium))
                x = F_loss.normalize(x.squeeze(1), dim=-1)  # (B, d)
                all_x.append(x.cpu())
        
        all_x = torch.cat(all_x, dim=0)
        d_model = all_x.shape[1]
        
        mu, kappa = compute_vmf_params(all_x, d_model)
        
        # Store in model
        self.model.encoder.vmf_signatures[task_id] = (mu, kappa)
        
        print(f"[OT-SIGN] Task {task_id} signature: kappa={kappa.item():.4f}, "
              f"mu_norm={mu.norm().item():.4f}, num_samples={all_x.shape[0]}")
        
        self.model.train()
        return mu, kappa
    # ===== End vMF signature computation =====
