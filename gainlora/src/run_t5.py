...
class ModelArguments:
    ...
    # ===== OT-SIGN model arguments =====
    use_ot_routing: bool = field(
        default=False,
        metadata={"help": "Enable OT-SIGN routing (vMF signatures + Sinkhorn OT)"}
    )
    ot_epsilon: float = field(
        default=0.05,
        metadata={"help": "Sinkhorn entropic regularization strength"}
    )
    ot_n_iter: int = field(
        default=10,
        metadata={"help": "Number of Sinkhorn iterations"}
    )
    default_kappa: float = field(
        default=10.0,
        metadata={"help": "Default kappa for current task's proxy in OT cost"}
    )
    previous_vmf_signatures_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to load vMF signatures from previous tasks"}
    )
...
class TrainingArguments(Seq2SeqTrainingArguments):
    ...
    lambda_inv: float = field(
        default=0.001,
        metadata={"help": "Weight for anti-invasion loss on current task data"}
    )
    invasion_threshold: float = field(
        default=2.3,
        metadata={"help": "vMF log-likelihood threshold for invasion detection (~-log(0.1))"}
    )
...
def main():
    ...
    prompt_config = {
        ...
        # OT-SIGN parameters
        'use_ot_routing': model_args.use_ot_routing,
        'ot_epsilon': model_args.ot_epsilon,
        'ot_n_iter': model_args.ot_n_iter,
        'default_kappa': model_args.default_kappa,
        'previous_vmf_signatures_path': model_args.previous_vmf_signatures_path,
    }
    ...
    if training_args.do_train:
        ...
        # ===== OT-SIGN: Compute and save vMF signature for current task =====
        if model_args.use_ot_routing and not prompt_config["run_single"]:
            mu, kappa = trainer.compute_vmf_signature(cur_task_id)
            
            # Save all signatures (previous + current)
            vmf_save_path = os.path.join(save_path, 'vmf_signatures.pt')
            torch.save(trainer.model.encoder.vmf_signatures, vmf_save_path)
            print(f"[OT-SIGN] Saved {len(trainer.model.encoder.vmf_signatures)} vMF signatures to {vmf_save_path}")
            
            # Log to run logger
            try:
                run_logger = RunLogger(
                    log_dir=os.path.join("logs_and_outputs", training_args.run_name, "ot_sign_logs")
                )
                run_logger.log_run(
                    task_name=cur_task,
                    task_id=cur_task_id,
                    config={
                        ...
                        "ot_n_iter": model_args.ot_n_iter,
                        "default_kappa": model_args.default_kappa,
                        "lambda_drift": training_args.lambda_drift,
                        "lambda_inv": training_args.lambda_inv,
                        "invasion_threshold": training_args.invasion_threshold,
                        "lora_r": model_args.lora_r,
                        "lora_alpha": model_args.lora_alpha,
                    ...
                    vmf_signature={"mu_norm": mu.norm().item(), "kappa": kappa.item()},
                    metrics=metrics,
                )
            except Exception as e:
                print(f"[OT-SIGN] Warning: Logger error: {e}")
        # ===== End OT-SIGN save =====
    ...
    if training_args.do_predict or training_args.do_train:
        ...
        if training_args.do_predict:
            ...
            # ===== Log predict scores for AP/FT computation =====
            try:
                task_order_list = [t.strip() for t in data_args.task_order.split(',') if t.strip()]
                _is_long = any(t in ['yelp', 'amazon', 'mnli', 'imdb', 'sst2', 'dbpedia', 'agnews'] for t in task_order_list)
                _btype = 'long' if _is_long else 'superni'
                _score_logger = RunLogger(
                    log_dir=os.path.join("logs_and_outputs", training_args.run_name, "ot_sign_logs"),
                    benchmark_type=_btype,
                )
                _score_logger.log_predict_scores(cur_task_id, metrics, task_order_list)
                # On final task, print AP/FT summary to console + file
                if cur_task_id == len(task_order_list) - 1:
                    _score_logger.print_ap_ft(task_order_list, method_name=training_args.run_name)
            except Exception as _e:
                print(f"[RunLogger] Score logging error: {_e}")
            # ===== End AP/FT logging =====
