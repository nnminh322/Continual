#!/bin/bash
###############################################################################
# OT-SIGN + GainLoRA (T5-Large) — Order 3 (Long Benchmark)
# ==========================================================
# Task order: yelp → amazon → mnli → ... → wic (15 tasks)
# Benchmark:  Long (Exact Match metric)
# Hardware:   2× T4 16GB  |  Estimated time: ~3-4h
# Run:        bash run_ot_sign_order3_t5large.sh [GPU_IDS] [MODEL_PATH]
###############################################################################
...
# ── Config ──────────────────────────────────────────────────────────────────
RUN_NAME="ot_sign_order3_t5large"
OUTPUT_BASE="logs_and_outputs/${RUN_NAME}/outputs"
CONFIG_PREFIX="configs/gen_script_long_order3_t5_configs"
GEN_DATA_DIR="generated_data/lora_gen_long_t5"
METRIC_TYPE="exact_match"   # Exact Match for Long benchmark
KL_RATIO=0.1
...
mkdir -p "${OUTPUT_BASE}"
echo "════════════════════════════════════════════════"
echo "  OT-SIGN+GainLoRA  |  Order 3  |  T5-Large"
