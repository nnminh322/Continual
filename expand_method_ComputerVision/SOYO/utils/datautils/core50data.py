...
class CORE50(object):
    ...
    def __init__(self, root='', preload=False, scenario='ni', cumul=False,
        ...
        # preload force check
        _bin_path = os.path.join(self.root, 'core50_imgs.bin')
        _npz_file = os.path.join(self.root, 'core50_imgs.npz')
        if os.path.exists(_bin_path) or os.path.exists(_npz_file):
            self.preload = True
    ...
    def __next__(self):
        ...
        batch = self.batch
        # print('__next__')
        # print(self.batch) # 0-7
        # print(scen)        # ni
        # print(self.nbatch[scen])  # 8
        ...
        if self.cumul: # no
            ...
            # print(run)    # 0
            train_idx_list = self.LUP[scen][run][batch]
            # print(len(self.LUP[scen])) # 10
            # for i in range(10):
            #     print(len(self.LUP[scen][i])) # all 9
            # for i in range(9):
        ...
        if self.preload: # no
            ...
            # loading imgs
            train_x = self.get_batch_from_paths(train_paths).astype(np.float32)
            # print(train_x.shape) # (14991, 128, 128, 3)
            # exit()
        import datetime;print('end', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"), '\n')
    ...
    def get_test_set(self):
        ...
        test_idx_list = self.LUP[scen][run][-1] # 加载最后一个分支，也就是45000的那个
