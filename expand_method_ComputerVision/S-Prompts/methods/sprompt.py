...
from utils.srt_gate import PooledMahalanobisGate
...
class SPrompts(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.all_keys = []
        self.gate_mode = str(args.get("gate_mode", "kmeans")).lower()
        self.gate_clusters = int(args.get("gate_clusters", 5))
        self.gate_batch_size = int(args.get("gate_batch_size", 2048))
        self._srt_gate = None

        if self.gate_mode in {"srt_ridge", "srt_oas", "srt_none"}:
            shrinkage = self.gate_mode.split("_", 1)[1]
            self._srt_gate = PooledMahalanobisGate(shrinkage=shrinkage)
        elif self.gate_mode != "kmeans":
            raise ValueError("Unknown gate_mode: {}".format(self.gate_mode))
    ...
    def _collect_task_features(self, dataloader):
        features = []
        ...
        return torch.cat(features, 0).cpu().detach().numpy()

    def clustering(self, dataloader):
        features = self._collect_task_features(dataloader)
        if self.gate_mode == "kmeans":
            clustering = KMeans(n_clusters=self.gate_clusters, random_state=0).fit(features)
            self.all_keys.append(torch.tensor(clustering.cluster_centers_).to(self._device))
            return

        self._srt_gate.add_task(features)

    def _select_task_kmeans(self, feature: torch.Tensor) -> torch.Tensor:
        taskselection = []
        ...
        return torch.vstack(taskselection).min(0)[1]

    def _select_task_srt(self, feature: torch.Tensor) -> torch.Tensor:
        selection = self._srt_gate.route(feature.cpu().detach().numpy())
        return torch.from_numpy(selection).to(feature.device)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                if self.gate_mode == "kmeans":
                    selection = self._select_task_kmeans(feature)
                else:
                    selection = self._select_task_srt(feature)
