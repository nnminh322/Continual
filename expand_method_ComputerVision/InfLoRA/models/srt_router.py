"""
SRT Router: Statistical Routing Theory implementation for ViT backbone.

Ported from code_srt_sgwi_v1/src/srt_router.py (the canonical implementation).

Two modes:
  'hard'     : ZCA whitening + L2. Equivalent to Pooled Mahalanobis.
               Fitted ONCE from pooled covariance when 2nd task is added.
               Re-shrinks all tasks toward pooled Σ before fitting ZCA.
               Re-whitens ALL tasks (μ AND Σ) with the fixed transform.
               argmin is always valid — all tasks use L2 in same whitened space.

  'dynamics' : No whitening. SRM global metric selection per task.
...
def metric_psr(
    ...
    """
    Probabilistic Subspace Routing (PSR) metric.
...
def ledoit_wolf_shrinkage(Sigma: np.ndarray, factor: float = 0.1) -> np.ndarray:
    ...
    sigma_mean = trace / d if d > 0 else 1.0
...
def pooled_shrinkage_target(
    ...
    """
    Compute optimal pooled shrinkage target for task t (Theorem 5).
...
def srm_metric_selection(
    ...
    """
    Structural Risk Minimization (SRM) from Theorem 7.
    ...
    CRITICAL: SRM must run AFTER all tasks (including current) are added
...
class SRT_Router:
    """
    Statistical Routing Theory Router for ViT backbone.

    Two modes (set via mode parameter):
      'hard'    : ZCA whitening + L2. No SRM. Matches routing_analysis experiment.
                  Whitening fitted on train embeddings, applied always.
                  Single fixed metric (L2 in whitened space = Pooled Mahalanobis).
                  Re-shrinks all tasks toward pooled Σ before ZCA fitting.
                  Re-whitens ALL tasks (μ AND Σ) with the fixed W.
    ...
    def __init__(
        ...
        """
        assert mode in ('hard', 'dynamics'), f"Unknown mode: {mode}"
    ...
    def _update_pooled(self, mu_t: np.ndarray, Sigma_t: np.ndarray, n_t: int):
        """Update running pooled mean and covariance via Welford–Hart."""
        if self._n_pool == 0:
    ...
    def add_task(self, task_id: int, embeddings: np.ndarray) -> TaskSignature:
        """
        Add a new task's statistical signature.

        HARD MODE — ZCA fit-once:
          1. Re-shrink ALL previous tasks toward updated pooled Σ (raw Σ base)
          2. When ≥2 tasks: fit ZCA ONCE from pooled Σ, re-whiten ALL tasks (μ AND Σ)
          3. Whiten current task with fixed ZCA

        DYNAMICS MODE — SRM:
          1. Pooled update + re-shrink
          2. SRM metric selection after all tasks added

        Args:
            task_id: integer task ID
            embeddings: (n_t, D) mean-pooled frozen backbone embeddings
        ...
        if not self._zca_fitted:
            ...
            # Re-whiten ALL previous tasks (from their raw centroids and raw Σ)
    ...
    def route(self, features: np.ndarray) -> np.ndarray:
        ...
        Args:
            features: (n, D) or (D,) — mean-pooled frozen backbone embeddings
        ...
        if self.mode == 'hard' and self._W_zca is not None:
            features = apply_whitening(features, self._mu_global, self._W_zca)
        ...
        for i, t_id in enumerate(task_list):
            dists[:, i] = self.signatures[t_id].distance(features)
        ...
        if self._route_debug_count % 1000 == 0 and n > 0:
            print(f"[SRT-ROUTE] mode={self.mode} n_tasks={T} "
    ...
    def load(self, path: str):
        ...
        self.mode = str(data.get('mode', 'hard'))
