...
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 1-task748_glucose_reverse_cause_event_detection already complete"
else
...
# ── TASK 2: task073_commonsenseqa_answer_generation ─────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/2-task073_commonsenseqa_answer_generation"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 2-task073_commonsenseqa_answer_generation already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 3: task1590_diplomacy_text_generation ──────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/3-task1590_diplomacy_text_generation"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 3-task1590_diplomacy_text_generation already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 4: task639_multi_woz_user_utterance_generation ──────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/4-task639_multi_woz_user_utterance_generation"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 4-task639_multi_woz_user_utterance_generation already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 5: task1572_samsum_summary ──────────────────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/5-task1572_samsum_summary"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 5-task1572_samsum_summary already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 6: task1687_sentiment140_classification ────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/6-task1687_sentiment140_classification"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 6-task1687_sentiment140_classification already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 7: task591_sciq_answer_generation ──────────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/7-task591_sciq_answer_generation"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 7-task591_sciq_answer_generation already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 8: task363_sst2_polarity_classification ────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/8-task363_sst2_polarity_classification"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 8-task363_sst2_polarity_classification already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 9: task1510_evalution_relation_extraction ──────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/9-task1510_evalution_relation_extraction"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 9-task1510_evalution_relation_extraction already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 10: task1729_personachat_generate_next ─────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/10-task1729_personachat_generate_next"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 10-task1729_personachat_generate_next already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 11: task181_outcome_extraction ─────────────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/11-task181_outcome_extraction"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 11-task181_outcome_extraction already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 13: task002_quoref_answer_generation ───────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/13-task002_quoref_answer_generation"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 13-task002_quoref_answer_generation already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
...
# ── TASK 14: task1290_xsum_summarization ─────────────────────────────────────
OUTPUT_DIR="$BASE_OUT/outputs/14-task1290_xsum_summarization"
if [ -f "$OUTPUT_DIR/done" ]; then
    echo "[SKIP] Task 14-task1290_xsum_summarization already complete"
else
    CUDA_VISIBLE_DEVICES=$GPU_IDS python src/run_llama.py \
       --do_train \
       --do_predict \
