"""
SpecRoute Trainer for Llama: Custom Seq2SeqTrainer for SpecRoute continual learning on Llama.

Key differences from GainLoRA_InfLoRA_Trainer (ROOT Llama):
- No trans_input GPM constraints (routing is parameter-free spectral projection)
- No memory replay (no KL loss — routing has no learned parameters)
- Simplified optimizer (no special lr for trans_input)
- ESA dynamic threshold for GPM

Key differences from T5 cl_trainer_specroute.py:
- Uses deepspeed with distributed gather/scatter for multi-GPU (Llama requirement)
- Llama-specific prediction_step with LLaMA-2 generation config
"""
...
class SpecRoute_Trainer(Seq2SeqTrainer):
    ...
    def training_step(self, model, inputs, **kwargs):
        """Standard CE training step with one-time gradient diagnostic. No memory replay."""
        model.train()
    ...
    def get_reg_matrix(self):
        ...
        Uses distributed scatter for multi-GPU deepspeed (same as ROOT Llama).
        No trans_input/prompt_key operations.
        """
        ...
        for name, module in self.model.named_modules():
            if hasattr(module, 'get_feature'):
                ...
                for index in self.feature_list[i].keys():
                    if dist.get_rank() == 0:
                        feature_mat_list = [
                            torch.mm(self.feature_list[i][index], self.feature_list[i][index].T).to("cuda:0")
    ...
    def get_repsentation(self):
        ...
        Uses distributed gather/scatter for multi-GPU deepspeed (same as ROOT Llama).
        ESA dynamic threshold. No trans_input features collected.
        """
        self.feature_list, self._cur_task = self.load_previous_reg_matrix()
        ...
        # Distributed gather of covariance matrices
        mat_list = []
        for name, module in self.model.named_modules():
            if hasattr(module, 'get_feature'):
                ...
                for index in range(module.index):
                    if dist.get_rank() == 0:
                        gathered_tensors = [
                            torch.zeros(*module.matrix[index].shape, device=cur_device)
                            for _ in range(dist.get_world_size())
                        ]
                    else:
                        gathered_tensors = None
                    dist.gather(module.matrix[index].to(cur_device).float(), gathered_tensors, dst=0)
                    if dist.get_rank() == 0:
        ...
        # No trans_input feature collection (SpecRoute has no trans_input)
