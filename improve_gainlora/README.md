...
For SpecRoute (the new routing implementation), use these scripts:

```bash
# SuperNI
bash gen_script_superni_order1_t5_specroute.sh 0 google-t5/t5-large
bash gen_script_superni_order2_t5_specroute.sh 0 google-t5/t5-large

# Long benchmark
bash gen_script_long_order3_t5_specroute.sh 0 google-t5/t5-large
bash gen_script_long_order4_t5_specroute.sh 0 google-t5/t5-large
```

Important behavior:
- The updated `*_specroute.sh` scripts auto-detect GPU count and VRAM.
- If 2xT4 is available, scripts automatically run with `CUDA_VISIBLE_DEVICES=0,1` and `--fp16`.
- The first argument (`0`) is only used for single-GPU fallback mode.
