#!/usr/bin/env python3
"""
Option A: HiDe-LLaVA Cosine → SRT — Routing Accuracy Evaluation.
...
def parse_args():
    parser = argparse.ArgumentParser(description="HiDe-LLaVA Cosine → SRT Routing Accuracy (GPU)")
    parser.add_argument("--data_root", type=str, required=True)
    parser.add_argument("--task_names", type=str, nargs="+", required=True)
    parser.add_argument("--clip_model", type=str,
                       default="openai/clip-vit-large-patch14-336")
    parser.add_argument("--output_dir", type=str, default="results_hide")
...
def main():
    ...
    print(f"\n{gpu_tag}  === CL Routing [HiDe-LLaVA Cosine → SRT] ===")
    ...
    # Load CLIP (GPU)
    extractor = CLIPVisionExtractor(model_name=args.clip_model, device=args.device, dtype="float16")
    ...
    for task_name in tqdm(args.task_names, desc="  Extracting"):
        ...
        if len(paths) < 2:
            print(f"  [WARN] {task_name}: {len(paths)} images")
            continue
        rng = np.random.RandomState(hash(task_name) % (2**32))
        idx = rng.permutation(len(paths))
        split = max(2, int(len(idx) * 0.8))
        train_p = [paths[i] for i in idx[:split]]
        test_p = [paths[i] for i in idx[split: split + args.n_test]]
    ...
    print(f"  SRT-Mahal_{args.shrinkage.upper()} (NEW) vs Cosine HiDe (ORIGINAL)")
    print(f"  Device: {args.device}")
    print(f"{'='*75}\n")

    # SRT router (GPU)
    srt_router = PooledMahalanobisRouter(shrinkage=args.shrinkage, device=args.device)
    print(f"  SRT Router on {srt_router.device}")

    all_results = []
    cosine_anchors = []  # HiDe-style running mean anchors
    ...
    for step_idx, task_name in enumerate(args.task_names):
        ...
        cosine_anchors.append(train_embs.mean(axis=0))
        ...
        for t_idx, seen_task in enumerate(args.task_names[:step_idx + 1]):
            ...
            # Cosine HiDe (ORIGINAL)
            C = np.stack(cosine_anchors)
        ...
        print("    Cosine (HiDe-LLaVA ORIGINAL)")
        print(f"    SRT-Mahal_{args.shrinkage.upper()} (NEW)")
        print(
            f"    RESULT Cosine (HiDe-LLaVA ORIGINAL)  macro={acc_cos*100:6.2f}%"
            f"  [{fmt(pt_cos)}]"
        )
        print(
    ...
    print(f"  {'Cosine HiDe (ORIGINAL)':30}  {fn('acc_cos')}  {av('acc_cos')}  {ts('acc_cos')}")
    ...
    out_path = output_dir / f"hide_cosine_srt_{args.shrinkage}_{args.device}.json"

    report = {
        "method": "HiDe-LLaVA Cosine → SRT",
        ...
        "results": {
            f"step_{i+1}": {
                ...
                "accuracy_cosine_hide": r["acc_cos"],
